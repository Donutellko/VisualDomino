...
namespace VisualDomino {
	...
	internal class Bot_Stoyanovskii : Player {
		public static string PlayerName = "SU_Bot";
		private List<MTable.SBone> _lHand;

		private static byte[] vars = { 20, 10, 8, 6, 4, 2, 0, 0 };

		//Переменные
		private List<MTable.SBone> _lsbUsed = new List<MTable.SBone>();

		private bool _pInit;
		private int _pEnemyBoneCount;
		private MTable.SBone _pLeft;
		private MTable.SBone _pRight;

		private int[,] _iProbabilityTable = {
			{7,7,7,7,7,7,7},
			{7,7,7,7,7,7,7},
			{7,7,7,7,7,7,7},
			{7,7,7,7,7,7,7},
		...
		public TurnOption GetBt (List<TurnOption> ltoUsefulDominoes) {
			bool flag = false;
